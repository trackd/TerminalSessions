...
/// </summary>
[Cmdlet(VerbsCommon.Get, "TerminalClientInfo", DefaultParameterSetName = "BySessionInfo")]
[Alias("Get-WTSClientInfo", "gtc")]
[OutputType(typeof(ClientInfo))]
public class GetTerminalClientInfo : PSCmdlet
{
  /// <summary>
  ...
  protected override void ProcessRecord()
  {
    ...
    {
      // We don't have state info, so always try
    ...
    foreach (var group in _sessions.GroupBy(s => s.ComputerName))
    {
      ...
      try
      {
        ...
        foreach (var session in group)
        {
          try
          {
            var info = WtsNative.WTSQueryClientInfo(serverInfo, session.SessionId);
            WriteObject(info);
          }
          catch (Exception ex)
          {
